...
def auth_pages():
    st.markdown("<h1 style='text-align: center; color: #6366f1;'>🏦 Digital Wallet</h1>", unsafe_allow_html=True)
            ...
            with st.form("login_form"):
                ...
                if submitted:
                    ...
                    if not user.empty:
                        ...
                        st.markdown("<div class='error-message'>⚠️ Invalid username or password</div>", unsafe_allow_html=True)
            ...
            <div class="txn-card {cls}">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="flex: 1; text-align: right;">
                        <span class="{amount_cls}"><strong>{sign}₨{amount:,.2f}</strong></span><br>
...
def create_expense_chart(df):
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        labels=df['Category'],
        colors=colors,
        autopct=lambda p: f'₨{p*sum(df["Amount"])/100:,.0f}\n({p:.1f}%)',
        startangle=90,
    ...
    legend_labels = [f"{row['Category']} (₨{row['Amount']:,.0f})" for _, row in df.iterrows()]
    legend = ax.legend(
        wedges,
        legend_labels,
        title="Categories",
...
def dashboard():
    ...
    if 'last_transaction' in st.session_state:
        ...
        emoji = "💸" if txn['type'] == 'send' else "📥" if txn['type'] == 'receive' else "🧾"
        st.markdown(f"""
    ...
    with col1:
        st.markdown(f"<h1 style='color: var(--text-color);'>🏦 Welcome, {user['username']}</h1>",
                    unsafe_allow_html=True)
    with col2:
    ...
    if st.session_state.get('show_add_funds'):
      with st.expander("💵 Add Funds", expanded=True):
          with st.form("add_funds"):
              ...
              if st.form_submit_button("💳 Add Funds"):
                  ...
                  st.session_state.last_transaction = {
                      'type': 'deposit',
                      'message': f"Added ₨{amount:,.2f} to your account"
                  }
    ...
    if st.session_state.get('show_send_money'):
      with st.expander("📤 Send Money", expanded=True):
            ...
            if st.form_submit_button("💸 Send Now"):
                ...
                if recipient_user.empty:
                    ...
                    if users_df.at[sender_idx, 'balance'] < amount:
                        ...
                        st.session_state.last_transaction = {
                            'type': 'send',
                            'message': f"Sent ₨{amount:,.2f} to {recipient_user.iloc[0]['username']}"
                        }
    ...
    if st.session_state.get('show_pay_bills'):
        with st.expander("🧾 Pay Bills", expanded=True):
            ...
            if service == "Electricity":
                ...
                with st.form("mobile_bill"):
                    ...
                    if st.form_submit_button("📱 Recharge"):
...
def process_bill_payment(service, provider, ref_no, amount):
    ...
    st.session_state.last_transaction = {
        'type': 'bill',
        'message': f"Paid ₨{amount:,.2f} to {provider} ({service})"
    }
